...
class ActivitiesPanel(wx.Panel):
    def __init__(self, parent, giface, settings, scaniface):
        ...
        self.configPath = filebrowse.FileBrowseButton(
            self,
            labelText="Configuration:",
            fileMask="JSON file (*.json)|*.json",
        ...
        self.buttonCalibrate = wx.Button(self, size=(150, -1), label="Scan/Calibrate")
        ...
        self.buttonCalibrate.Bind(
            wx.EVT_BUTTON, lambda evt: self.Calibrate(startTask=False)
        )
        self.buttonStop.Bind(wx.EVT_BUTTON, self.OnStop)
        self.timeText = wx.StaticText(self, label="00:00", style=wx.ALIGN_CENTRE)
        self.timeText.SetFont(
            wx.Font(15, wx.FONTFAMILY_TELETYPE, wx.NORMAL, wx.FONTWEIGHT_BOLD)
        )
        self.slidesStatus = wx.StaticText(
            self, label="Slides off", style=wx.ALIGN_CENTRE | wx.ALIGN_CENTRE_HORIZONTAL
        )
        ...
        sizer.Add(
            self.configPath, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        ...
        sizer.Add(
            self.buttonBack, proportion=0, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        sizer.Add(
            self.buttonForward, proportion=0, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        self.mainSizer.Add(sizer, flag=wx.EXPAND | wx.ALL, border=5)
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            self.buttonStart, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        #        sizer.Add(self.buttonPause, proportion=1, flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL, border=5)
        sizer.Add(
            self.buttonStop, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        sizer.Add(
            self.slidesStatus,
            proportion=0,
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            self.buttonNext, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        self.mainSizer.Add(sizer, flag=wx.EXPAND | wx.ALL, border=5)
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            self.instructions, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL, border=5
        )
        self.mainSizer.Add(sizer, flag=wx.EXPAND | wx.ALL, border=5)
    ...
    def _init(self):
        ...
        if self.configFile:
            ...
            if (
                "choices" in self.tasks[self.current]
                and self.tasks[self.current]["choices"]
            ):
    ...
    def _bindUserStop(self):
        ...
        bindings = {
            "stopTask": self.OnUserStop,
            "scanOnce": self.OnScanOnce,
            "taskNext": self.OnNextTask,
            "taskPrevious": self.OnPreviousTask,
            "startTask": self.StartAutomated,
        }
        if "keyboard_events" in self.configuration:
            ...
            for eventName in self.configuration["keyboard_events"]:
                ...
                items.append(
                    (
                        wx.ACCEL_NORMAL,
                        self.configuration["keyboard_events"][eventName],
                        eventId,
                    )
                )
                for win in windows:
    ...
    def _checkChangeTask(self):
        if self.timer.IsRunning():
            dlg = wx.MessageDialog(
                self,
                "Stop currently running task before changing task",
                "Stop task",
    ...
    def _loadConfiguration(self, event):
        def _includeTasks():
            ...
            if "includeTasks" in self.configuration:
                folder = os.path.join(
                    self._getTaskDir(), self.configuration["includeTasks"]
                )
                        ...
                        if file_ != os.path.basename(self.configFile):
                            with open(os.path.join(folder, file_), "r") as f:
                                try:
                                    ...
                                    wx.MessageBox(
                                        parent=self,
                                        message="Parsing error while reading %s."
                                        % file_,
    ...
    def Calibrate(self, startTask):
        ...
        self.settings["analyses"]["file"] = os.path.join(
            self._getTaskDir(), self.tasks[self.current]["analyses"]
        )
    ...
    def _startSlides(self):
        ...
        self.slides.LoadURL(
            "file://"
            + os.path.join(
                self.configuration["slides"]["dir"],
                self.tasks[self.current]["slides"]["file"],
            )
        )
        ...
        wx.CallLater(
            self.tasks[self.current]["slides"]["switch"][-1] * 1000, self._startTask
        )
    ...
    def _getTaskDir(self):
        return (
            self.configuration["taskDir"]
            if "taskDir" in self.configuration
            else os.path.dirname(self.settings["activities"]["config"])
        )

    def _getInstructions(self):
        instr = (
            self.configuration["tasks"][self.current]["instructions"]
            if "instructions" in self.configuration["tasks"][self.current]
            else ""
        )
    ...
    def _startTask(self):
        ...
        if "filter" in self.tasks[self.current]:
            ...
            self.scaniface.filter["threshold"] = self.tasks[self.current]["filter"][
                "threshold"
            ]
            self.scaniface.filter["debug"] = self.tasks[self.current]["filter"]["debug"]
        if (
            "single_scan" in self.tasks[self.current]
            and self.tasks[self.current]["single_scan"]
        ):
            self._stopScanning()
        else:
            self._startScanning()
    ...
    def OnTimer(self, event):
        ...
        if "time_limit" in self.tasks[self.current] and diff > datetime.timedelta(
            seconds=self.tasks[self.current]["time_limit"]
        ):
            self.timer.Stop()
    ...
    def LoadLayers(self):
        ...
        for i, cmd in enumerate(self.configuration["tasks"][self.current]["layers"]):
            ...
            if "layers_opacity" in self.configuration["tasks"][self.current]:
                opacity_list = self.configuration["tasks"][self.current][
                    "layers_opacity"
                ]
                if i < len(opacity_list):
                    opacity = float(opacity_list[i])
                else:
                    self.giface.WriteWarning(
                        "Number of layers is larger than the number of opacity values in config file"
                    )
            if "layers_checked" in self.configuration["tasks"][self.current]:
                checked_list = self.configuration["tasks"][self.current][
                    "layers_checked"
                ]
                if i < len(checked_list):
                    ...
                    self.giface.WriteWarning(
                        "Number of layers is larger than the number of checked values in config file"
                    )
            if cmd[0] == "d.rast":
                lr = ll.AddLayer(
                    "raster",
    ...
    def LoadHandsOff(self):
        ...
        self.handsoff = ll.AddLayer(
            "command", name=" ".join(cmd), checked=True, opacity=1.0, cmd=[]
        )
    ...
    def StartDisplay(self):
        title = (
            None
            if "title" not in self.tasks[self.current]["display"]
            else self.tasks[self.current]["display"]["title"]
        )
        vertical = (
            False
            if "vertical" not in self.tasks[self.current]["display"]
            else self.tasks[self.current]["display"]["vertical"]
        )
    ...
    def _showSolutions(self):
        ...
        if cmd[0] == "d.rast":
            ll.AddLayer(
                "raster", name=cmd[1].split("=")[1], checked=True, opacity=1.0, cmd=cmd
            )
        elif cmd[0] == "d.vect":
            ll.AddLayer(
                "vector", name=cmd[1].split("=")[1], checked=True, opacity=1.0, cmd=cmd
            )
